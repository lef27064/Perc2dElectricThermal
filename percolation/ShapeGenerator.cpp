...
void ShapeGenerator::areaSolve(void)
{
	...
	do
	{
		...
		// Compute ΣAi*ρi (sum of component area times special weight)
		for (int i = 0; i < totalComponents; i++)
		{
			sum = sum + components[i] * specialWeights[i];
		}

		// Calculate current Wi = (Ai*ρi) / ΣAj*ρj and compute errors
		...
		// Find Ai = W * ΣAj*ρj / ρi (re-calculate component areas)
		for (int i = 0; i < totalComponents; i++)
		{
...
void ShapeGenerator::monteCarlo(void)
{
	...
	for (int i = 0; i < iterations; i++) // Loop for each Monte Carlo iteration
	{
        ...
        //Edmonds_karp solver;
        //std::vector<std::vector<std::vector<int>>> Edmonds_karpPaths;
        // Καλούμε τη συνάρτηση για να βρούμε τα μονοπάτια
        

       // diffPaths[i] = solver.find_paths(grid->cell, this->height, this->width, Edmonds_karpPaths);
        // Δημιουργία νέου δυναμικού πίνακα
        //char* grid_copy = new char[grid->width * grid->height];

        // Αντιγραφή των δεδομένων από το grid->cell στον grid_copy
        // Η memcpy είναι πολύ πιο γρήγορη από έναν απλό βρόχο
        //memcpy(grid_copy, grid->cell, grid->width * grid->height * sizeof(char));
        ...
        //Dinicomp solver(this->height, this->width);
        //diffPaths[i] = solver.max_flow(grid_copy, 1);

        //SimplePathFinder finder(grid_copy, this->height, this->width);
        //diffPaths[i] = finder.find_all_paths();
        // Stop the clock.
        //auto end = std::chrono::high_resolution_clock::now();

        // Calculate the duration.
        //std::chrono::duration<double> duration = end - start;
        // Αποδέσμευση της μνήμης του αντιγράφου
		...
		if (calcElectricConductivity)
		{
			...
			Results[i] = grid->percolateWithRealPathLength(&paths[i], &meanPathLength[i],
				...
				&this->meanRVEYoungModulus[i], materialsShearModulus, &this->meanRVEShearModulus[i], materialsPoissonRatio, &meanRVEPoissonRatio[i]);
            //std::cout << "Maximum paths with Dinic=" << diffPaths[i] << std::endl;
            //std::cout << "Path Diferences (current vs Dinic)=" << paths[i]-diffPaths[i] << std::endl;
            //std::cout << "Time elapsed Dinic:" << duration.count() << " seconds" << std::endl;
		...
		if (paths[i] > 0)
		{
			...
			PoissonRatio[i] = meanRVEPoissonRatio[i];               // Poisson's ratio
		...
		if (settings->saveImageFile)
		{
			...
			if (!settings->RandomSaveImageFile) // Save at fixed intervals
			{
				if (i % (iterations / settings->totalImagesToSave) == 0) // Check if it's time to save
				{
					...
					grid->saveToDisk(&FileName[0u], settings->imageType); // Save grid to disk
					if (calcStatistcs)
					{
	...
	*/
	...
	if (meanPercolation > 0) // If there is percolation
	{
        ...
        meanShearModulus = pow((materialsShearModulus[1] * meanRVEPaths * meanRVEPathWidth), (width / meanRVEPathLength)) + (materialsShearModulus[0] * (width - (meanRVEPaths * meanRVEPathWidth)) / width);
		// Estimate Poisson's Ratio using a mixture rule
		meanPoissonRatio = meanYoungModulus / (2 * meanShearModulus) - 1;
	}
	else // If no percolation
	{
		...
		meanShearModulus = materialsShearModulus[0];
	}

	// Print final results
	cout << string(42,'-') << " Results(RSE)  " << string(42, '-') << endl;;
	...
	cout << "Mean Shear Modulus =" << meanShearModulus << "\n";
...
void ShapeGenerator::saveResultstoReport(ReportType ireportType)
{
    ...
    if (calcElectricConductivity)
        File << seperator << "Electric Conductivity" << seperator << "Thermal Conductivity" << seperator << "Young Modulus" << seperator << "Shear Modulus" << seperator << "Poisson Ratio"
    ...
    for (int i = 0; i < iterations; i++)
    {
        ...
        if (calcElectricConductivity)
            ...
            << seperator << setw(13) << std::fixed << std::setprecision(std::numeric_limits<double>::max_digits10) << this->meanRVEShearModulus[i]
            << seperator << setw(13) << std::fixed << std::setprecision(std::numeric_limits<double>::max_digits10) << this->meanRVEPoissonRatio[i]
    ...
    if (inputFile.is_open()) { // Check if the file was opened successfully
        ...
        while (std::getline(inputFile, line)) {
            ...
            for (int i = 0; i < totalComponents; i++) {
				...
				//std::cout << "Material " << i << ": Young's Modulus = " << materialsYoungModulus[i] << "\n";
            }
            ...
            for (int i = 0; i < totalComponents; i++) {
				...
				//std::cout << "Material " << i << ": Poisson's Ratio = " << materialsPoissonRatio[i] << "\n";
            }

            for (int i = 0; i < totalComponents; i++) {
                materialsShearModulus[i] = materialsYoungModulus[i] / (2 * (1 + materialsPoissonRatio[i]));
				//std::cout << "Material " << i << ": Shear Modulus = " << materialsShearModulus[i] << "\n";
            }
